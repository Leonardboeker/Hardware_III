...
class TouchDesignerUiStateTests(unittest.TestCase):
    def test_build_ui_state_for_masonry_exposes_phase_path_text(self) -> None:
        ...
        self.assertIn("CHOOSE PHASE", ui_state["panel_texts"]["top_phase_navigation"])
        self.assertIn("1 Foundation", ui_state["panel_texts"]["top_phase_navigation"])
        self.assertIn("YOUR SELECTION", ui_state["panel_texts"]["left_info"])
        self.assertIn("MASONRY", ui_state["panel_texts"]["method_selection"])
    ...
    def test_build_ui_state_for_prefab_exposes_lifecycle_mode_and_sub_method(self) -> None:
        ...
        self.assertIn("Lifecycle mode active", ui_state["panel_texts"]["top_phase_navigation"])
        self.assertIn("Production", ui_state["panel_texts"]["right_phase_preview"])
        self.assertIn("PREFAB", ui_state["panel_texts"]["method_selection"])
        self.assertIn("CLT", ui_state["panel_texts"]["right_phase_preview"])
        self.assertEqual(
    ...
    def test_compute_and_store_touchdesigner_ui_writes_panel_text_storage(self) -> None:
        ...
        self.assertIn("3 Roof", owner._storage["text_right_phase_preview"])
