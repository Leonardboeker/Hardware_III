    ...
    if not os.path.exists(_MODEL_PATH):
        ...
        try:
            ...
            if os.path.exists(_MODEL_PATH):
                os.remove(_MODEL_PATH)   # remove partial file
...
class GestureDetector:
    def __init__(self):
        ...
        self._grace_gesture  = None   # gesture held during grace period
        self._grace_start    = None   # time grace period began
        self._grace_frames   = 0      # frames since hand disappeared
        self._t0 = time.time()   # reference time for monotonic timestamps
    ...
    def process(self, frame):
        ...
        if result.hand_landmarks:
            self._grace_frames = 0   # hand is visible, reset grace counter
            ...
            # Hand disappeared — allow up to 3 frames grace before resetting
            self._grace_frames += 1
            if self._grace_frames <= 3:
                # Carry forward the last gesture and timer so one-frame occlusions
                # (sleeve, reflection, projector glare) don't break dwell holds.
                gesture = self._current or 'none'
                dwell   = now - self._start if self._start else 0.0
            else:
                # Grace period expired — treat as gesture release
                if self._current == 'fist' and self._start is not None:
                    held = now - self._start
                    if held >= DWELL_RESET:
                        actions.append('reset')
