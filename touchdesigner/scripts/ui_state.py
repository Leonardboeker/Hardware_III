...
DEFAULT_LIFECYCLE_CHIP_LABELS = ("A1-A3", "A4", "A5", "B", "C")
METHOD_CARD_BLOCK_IDS = (
    "method_card_masonry",
    "method_card_3d_printed",
    "method_card_prefab",
)
FLOOR_CONTROL_BLOCK_IDS = (
    "left_info_scale",
    "left_info_scale_minus",
    "left_info_scale_value",
    "left_info_scale_plus",
)
COMPARISON_REVIEW_BLOCK_IDS = (
...
def _compact_navigation_label(
    ...
    compact = str(stage_label)
    replacements = {
        "2 Structure / Walls": "2 Walls",
        "Structure / Walls": "Walls",
    }
    for old, new in replacements.items():
...
def _phase_navigation_payload(
    metrics_output: dict[str, Any] | None,
    *,
    method_key: str | None = None,
    active_stage_key: str | None = None,
) -> dict[str, Any]:
    payload = dict(metrics_output or {})
    data_model = payload.get("data_model")
    is_lifecycle = data_model == "lifecycle_based" or method_key == "prefab"
    default_labels = DEFAULT_LIFECYCLE_CHIP_LABELS if is_lifecycle else DEFAULT_PHASE_CHIP_LABELS
    stage_sequence = list(payload.get("stage_sequence") or [])
    stage_labels = payload.get("stage_labels", {})

    if not stage_sequence:
        stage_sequence = list(DEFAULT_LIFECYCLE_CHIP_LABELS if is_lifecycle else (
            "foundation",
            "structure",
            "roof",
            "openings",
    ...
    items = []
    for index, stage_key in enumerate(stage_sequence[: len(PHASE_NAV_BLOCK_IDS)]):
        stage_label = stage_labels.get(stage_key, default_labels[index] if index < len(default_labels) else stage_key)
        items.append(
            {
                "block_id": PHASE_NAV_BLOCK_IDS[index],
                "stage": stage_key,
                "label": _compact_navigation_label(stage_key, stage_label, data_model),
                "active": stage_key == active_stage_key,
                "disabled": not bool(method_key),
                "mode": "lifecycle" if is_lifecycle else "phase",
            }
        )

    while len(items) < len(PHASE_NAV_BLOCK_IDS):
        block_id = PHASE_NAV_BLOCK_IDS[len(items)]
        items.append(
            {
                "block_id": block_id,
                "stage": None,
    ...
    if not method_key:
        note = "Select a method, then set floors to unlock stages."
    elif is_lifecycle:
        note = "Lifecycle mode active. Phase scroller can move through CLT and modular prefab tracks."
    else:
        note = "Use the phase scroller to inspect each construction stage."
...
def _guidance_payload(
    ...
    phase_navigation: dict[str, Any],
) -> dict[str, Any]:
    payload = dict(metrics_output or {})
    scenario = payload.get("scenario", {})
    method_key = scenario.get("construction_method") or live_state.get("current_method")
    data_model = payload.get("data_model")
    selected_zone_ids = list(live_state.get("selected_zone_ids") or [])
    floor_dirty = bool(live_state.get("floor_control_dirty") or 0)
    phase_dirty = bool(live_state.get("phase_control_dirty") or 0)
    building_part_dirty = bool(live_state.get("building_part_interacted") or 0)
    phase_blocks = [
        str(item["block_id"])
        for item in phase_navigation.get("items", [])
        if item.get("label")
    ]

    if not method_key:
        return {
            "target": "method_selection",
            "message": "Choose a construction method to unlock the path.",
            "highlight_blocks": ["method_selection", *METHOD_CARD_BLOCK_IDS],
        }

    if not floor_dirty:
        return {
            "target": "floor_control",
            "message": "Set the building scale with the floor control or hardware scroller.",
            "highlight_blocks": ["left_info", "left_info_scale", *FLOOR_CONTROL_BLOCK_IDS],
        }

    if not phase_dirty:
        if data_model == "lifecycle_based" or method_key == "prefab":
            return {
                "target": "phase_navigation",
                "message": "Use the lifecycle scroller to move through CLT and modular prefab stages.",
                "highlight_blocks": ["top_phase_navigation", *phase_blocks, "method_card_prefab"],
            }
        return {
            "target": "phase_navigation",
            "message": "Use the phase scroller to inspect each construction stage.",
            "highlight_blocks": ["top_phase_navigation", *phase_blocks],
        }

    if not building_part_dirty and not selected_zone_ids:
        return {
            "target": "main_plan_simulation",
            "message": "Click a building part to localize the impact.",
            "highlight_blocks": ["main_plan_simulation"],
        }

    return {
        "target": "impact_review",
        "message": "Review local impact here, then compare it against the total project panels.",
        "highlight_blocks": [
            "right_comparison",
            "right_cost_chart",
            "right_phase_preview",
...
def _build_panel_texts(
    ...
    phase_navigation = _phase_navigation_payload(
        metrics_output,
        method_key=method_key,
        active_stage_key=active_stage.get("stage"),
    )
    guidance = _guidance_payload(metrics_output, live_state, phase_navigation)
    ...
    selected_parts_count = len(selected_zone_ids) if selected_zone_ids else int(
        live_state.get("selected_parts_count") or 0
    )
    whole_building_active = (
        not selected_zone_ids
        and (
            selected_parts_count <= 0
            or selected_part_label == "Whole Building"
        )
    )
    scope_value = (
        "Whole Building"
        if whole_building_active
        else f"{selected_parts_count} selected"
    )
    ...
    bar_bottom_status = (
        ...
        f"| 4 CLICK BUILDING PART || NEXT: {guidance['message'] or next_action} || VISION {hb_label}"
    )
    ...
    for item in phase_navigation["items"]:
        panel_texts[str(item["block_id"])] = str(item["label"])
    return panel_texts
...
def build_ui_state(
    ...
    if not metrics_output:
        ...
        phase_navigation = _phase_navigation_payload(None)
        guidance = _guidance_payload(None, live_state, phase_navigation)
        ...
        return {
            ...
            "warning_labels": [],
            "phase_navigation_mode": phase_navigation["mode"],
            "phase_navigation_note": phase_navigation["note"],
            "phase_navigation_items": phase_navigation["items"],
            "guidance_target": guidance["target"],
            "guidance_message": guidance["message"],
            "guidance_highlight_blocks": guidance["highlight_blocks"],
...
def live_state_from_touchdesigner(owner: Any | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "floor_control_dirty": owner.fetch("floor_control_dirty", 0),
        "phase_control_dirty": owner.fetch("phase_control_dirty", 0),
        "building_part_interacted": owner.fetch("building_part_interacted", 0),
    }
